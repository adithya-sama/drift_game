...
public class level_2_handler_old: MonoBehaviour
{
    public int skip_to_sequence;
    public camera_follow_player camera_follow_player_instance;
    public mouse_event_watcher mouse_event_watcher_instance;
    public GameObject player, deploy_minions_animation_obj, minions_obj, key_obj, level_name_obj, level_transition_out;
    public big_door_handler left_door, right_door;
    public static level_2_handler_old instance {get; private set;}
    public GameObject[] key_minion_pointer = new GameObject[4];
    public GameObject[] dialogs, on_screen_controls;
    ...
    void Awake(){
        ...
        if(skip_to_sequence != -100){
            current_sequence = skip_to_sequence;
            start_next_sequence(skip_to_sequence - 1);
        }
    }
    void Start(){
    ...
    public void start_next_sequence(int prev_sequence){
        ...
        if(current_sequence == 0 && prev_sequence == -1){
            ...
            for(int i=0; i < on_screen_controls.Length; i++){
                on_screen_controls[i].SetActive(true);
            }
        ...
        // minion deploy animation end
            ...
            level_transition_out.SetActive(true);
            current_sequence = 11;
